...
class ComposeSalad extends React.Component {
        ...
        */
        ...
        ingredientCheckbox(ingredient, name, ...rest) {
            ...
            return ( <
                SaladCheckbox key = { ingredient }
                name = { name }
                ingredient = { ingredient }
                cost = { cost }
                addBox = { this.addCheckbox.bind(this) }
                rest = { rest }
                />
            );
        }
                    ...
                    */
                    ...
                    render() {
                        ...
                        return ( <
                            ...
                            noValidate >
                            <
                            h4 > Välj bas < /h4> { this.foundationWarning() /* Triggers if no radio has been chosen */ } <
                            div className = "form-group" >
                            <
                            SaladRadioList ingredients = { this.foundations }
                            inventory = { this.props.inventory }
                            addRadioList = { this.addRadioList.bind(this) }
                            validate = { this.validateFoundation }
                            /> <
                            br > < /br> <
                            /div>
                            ...
                            <
                            h4 > Välj protein < /h4> <
                            div className = "form-group" > { this.proteins.map(ingr => this.ingredientCheckbox(ingr, "proteins")) } <
                            br > < /br> <
                            /div>

                            <
                            h4 > Välj tillbehör < /h4> <
                            div className = "form-group" > { this.extras.map(ingr => this.ingredientCheckbox(ingr, "extras")) } <
                            br > < /br> <
                            /div>

                            <
                            h4 > Välj dressing < /h4> { this.dressingWarning() } <
                            div className = "form-group requiredCheckboxes"
                            id = "dressingCheckboxes" > { this.dressings.map(ingr => this.ingredientCheckbox(ingr, "dressings", { validate: this.validateDressings })) } <
                            br > < /br> <
                            /div> <
                            input type = "submit"
